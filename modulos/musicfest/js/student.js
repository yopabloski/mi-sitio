...
function toggleArtist(id){const dayId=days[currentDay].id,ids=draft.selections[dayId],pos=ids.indexOf(id);if(pos>=0)ids.splice(pos,1);else{if(session.mode==='simultaneous')days.forEach(d=>{draft.selections[d.id]=draft.selections[d.id].filter(x=>x!==id)});ids.push(id)}draft.statuses[dayId]='editable';setSync('Guardando…');saveDraft(session.code,draft);setTimeout(()=>setSync(backend==='firebase'?'Sincronizado':'Modo local'),250);render()}
...
pintarOrden();
$('#submit').onclick=()=>{const count=session.mode==='simultaneous'?days.reduce((s,d)=>s+draft.selections[d.id].length,0):currentIds().length;$('#confirmText').textContent=session.mode==='simultaneous'?`Se bloquearán los tres días con ${count} artistas asignados.`:`Se cerrará ${days[currentDay].name} y sus ${count} artistas dejarán de estar disponibles en las rondas siguientes.`;$('#confirm').showModal()};$('#cancelSubmit').onclick=()=>$('#confirm').close();$('#confirmSubmit').onclick=()=>{const targetDays=session.mode==='simultaneous'?days:[days[currentDay]],pool=catalogArtists();draft.submissions=draft.submissions||{};targetDays.forEach(d=>{const result=validate(draft.selections[d.id],d,pool),dayIndex=days.findIndex(x=>x.id===d.id);draft.statuses[d.id]='submitted';draft.submissions[d.id]={dayId:d.id,dayIndex,dayName:d.name,selections:[...draft.selections[d.id]],totals:result.totals,checks:result.checks,revision:session.revision,validationStatus:'pending',submittedAt:new Date().toISOString()}});saveDraft(session.code,draft);$('#confirm').close();render()};

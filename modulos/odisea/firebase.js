...
if(enabled){
 ...
 api={
  ...
  adminLogin:()=>signInWithPopup(auth,new GoogleAuthProvider()),
  ...
  watchAttempts:(code,cb)=>onSnapshot(query(collection(db,'experiences',normalizeCode(code),'attempts'),orderBy('createdAt','desc')),s=>cb(s.docs.map(d=>({id:d.id,...d.data()})))),
...
export const store={
 ...
 watchAttempts:(code,cb)=>{if(enabled)return api.watchAttempts(code,cb);const emit=()=>cb(demoRead(code)?.attempts||[]);const handler=e=>{if(e.detail.code===normalizeCode(code))emit()};bus.addEventListener('change',handler);const storage=e=>{if(e.key===localKey(code))emit()};addEventListener('storage',storage);emit();return()=>{bus.removeEventListener('change',handler);removeEventListener('storage',storage)}},

...
if(enabled){
 ...
 api={
  ...
  async join(code,t){const experience=await this.load(code);if(!experience?.active)throw new Error('Esta actividad no existe o todavía no ha comenzado.');const user=auth.currentUser||(await signInAnonymously(auth)).user,id=normalizeCode(t.name)+'-'+user.uid.slice(0,6);await setDoc(doc(db,'experiences',normalizeCode(code),'teams',id),{...t,uid:user.uid,updatedAt:serverTimestamp()},{merge:true});return{...t,id,uid:user.uid,experienceId:normalizeCode(code)}},
